...
const Promo = (props) => {
  ...
  const renderPromo = () => {
    ...
    return promoItems[id].map((item, i) => {
      ...
      // If item has predefined pageURL, use it as it is
      if (item.pageUrl !== '') {
        pushRoute = item.pageUrl;
        // If item has groupItemIds, it's most likely category/serie item, redirect to categories view
        // } else if (item.groupItemIds.length > 0) {
        //   pushRoute = `/${routes.categories}/${organizationId}/${item.id}`;
        //   // If item.isSerie is true, it's episode so redirect to serieRoute
        // } else if (item.isSerie === true) {
        pushRoute = `/${routes.serieRoute}/${organizationId}/${item.id}`;
        // Else item is normal video asset, redirect to videoroute
      } else {
      ...
      return (
        <RenderItem
          key={`${id} ${i}`}
          item={item}
          pushRoute={pushRoute} //${item.seriesId}/${item.series[0].id}`}
          playOnClick={true}
